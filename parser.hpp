...
class parser {
    ...
    static std::vector<float> parse_vec(const toml::array &arr, int start,
                                        int cnt) {
    ...
    static auto rotate(const std::array<double, 3> &vert, const vec3_t &rot) {
        ...
        std::array<std::array<double, 3>, 3> rot_mat = {
            std::array<double, 3>{cy * cz, sx * sy * cz - cx * sz,
                                  cx * sy * cz + sx * sz},
            {cx * sz, sx * sy * sz + cx * cz, cx * sy * sz - sx * cz},
            {-sy, sx * cy, cx * cy}};
        for (int i = 0; i < 3; i++) {
            res[i] = std::inner_product(vert.begin(), vert.end(),
                                        rot_mat[i].begin(), 0.0);
        }
        return res;
    }

    static void transform_mesh(std::vector<std::array<double, 3>> &vertices,
                               transf &trans) {
        using vert_t = std::array<double, 3>;
        const auto num_vert = vertices.size();
        auto point_add = [](const vert_t &vert1, const vert_t &vert2) {
            return vert_t{vert1[0] + vert2[0], vert1[1] + vert2[1],
                          vert1[2] + vert2[2]};
        };
        auto center =
            std::reduce(std::execution::par, vertices.begin(), vertices.end(),
        ...
        auto distance = [](const vert_t &v1, const vert_t &v2) {
            const auto d0 = v1[0] - v2[0], d1 = v1[1] - v2[1],
                       d2 = v1[2] - v2[2];
            return std::sqrt(d0 * d0 + d1 * d1 + d2 * d2);
        };
        const auto farmost = std::max_element(
            std::execution::par, vertices.begin(), vertices.end(),
...
public:
    ...
    [[nodiscard]] world_t make_scene() const {
        ...
        for (auto &&tex : textures) {
            ...
            if (tex_type == "image") {
                ...
                tex_tbl.emplace(tex_name, std::make_shared<image_texture>(
                                              image_path.string()));
            } else if (tex_type == "solid") {
                const auto rgb = parse_vec(tex_info, 2, 3);
                tex_tbl.emplace(tex_name, std::make_shared<solid_color>(
                                              rgb[0], rgb[1], rgb[2]));
            } else {
        ...
        for (auto &&mat : materials) {
            ...
            if (mat_type == "lambertian") {
                if (use_tex) {
                    const auto tex_name =
                        mat_info[3].value<std::string>().value();
                    mat_tbl.emplace(mat_name, std::make_shared<lambertian>(
                                                  tex_tbl.at(tex_name)));
                } else {
                    const auto rgb = parse_vec(mat_info, 3, 3);
                    mat_tbl.emplace(mat_name,
                                    std::make_shared<lambertian>(
                                        vec3_t{rgb[0], rgb[1], rgb[2]}));
                }
                ...
                if (use_tex) {
                    ...
                    mat_tbl.emplace(mat_name,
                                    std::make_shared<dielectric>(ref_idx));
                }
                ...
                if (use_tex) {
                    ...
                    mat_tbl.emplace(mat_name, std::make_shared<metal>(
                                                  tex_tbl.at(tex_name), fuzz));
                } else {
                    const auto rgbf = parse_vec(mat_info, 3, 4);
                    mat_tbl.emplace(
                        mat_name,
                        std::make_shared<metal>(
                            vec3_t{rgbf[0], rgbf[1], rgbf[2]}, rgbf[3]));
                }
            } else if (mat_type == "light") {
                if (use_tex) {
                    const auto tex_name =
                        mat_info[3].value<std::string>().value();
                    mat_tbl.emplace(mat_name, std::make_shared<light>(
                                                  tex_tbl.at(tex_name)));
                } else {
                    const auto rgb = parse_vec(mat_info, 3, 3);
                    mat_tbl.emplace(mat_name, std::make_shared<light>(color_t{
                                                  rgb[0], rgb[1], rgb[2]}));
                }
            } else {
        ...
        if (config.contains("spheres")) {
            ...
            for (auto &&sph : spheres) {
                ...
                world.push_back(std::make_shared<sphere>(
                    vec3_t{xyzr[0], xyzr[1], xyzr[2]}, xyzr[3], sph_mat));
            }
        }
        ...
        if (config.contains("triangles")) {
            ...
            for (auto &&tri : triangles) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    tex_coords[i * 2] = vert[3],
                                   tex_coords[i * 2 + 1] = vert[4];
                }
                ...
                world.push_back(std::make_shared<triangle>(
                    vertices[0], vertices[1], vertices[2], tex_coords,
                    tri_mat));
            }
        }
        ...
        if (config.contains("meshes")) {
            ...
            for (auto &&mesh : meshes) {
                ...
                for (auto &&index : indices) {
                    ...
                    world.push_back(
                        std::make_shared<triangle>(p0, p1, p2, tex, mesh_mat));
                }
            }
        }
        ...
        if (config.contains("rectangles")) {
            ...
            for (auto &&rect : rectangles) {
                ...
                world.push_back(std::make_shared<rectangle>(
                    points[0], points[1], points[2], rect_mat));
            }
        }
    ...
    [[nodiscard]] tracer make_tracer() const {
        ...
        const auto bkg_vec =
            parse_vec(*config["canvas"]["background"].as_array(), 0, 3);
        const color_t bkg{bkg_vec.data()};
        const int samples_per_pixel =
            config["options"]["samples_per_pixel"].value<int>().value();
        const int max_depth =
            config["options"]["max_depth"].value<int>().value();
        const bool use_bvh = config["options"]["use_bvh"].value<bool>().value();
        const bool parallel =
            config["options"]["parallel"].value<bool>().value();
        tracer::config tconfig{width,     height,  samples_per_pixel,
        ...
        const auto lookfrom =
            parse_vec3(*config["camera"]["lookfrom"].as_array());
        ...
        const float aperture =
            config["camera"]["aperture"].value<float>().value();
