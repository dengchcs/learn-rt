...
public:
    ...
    virtual bool scatter(const ray &ray_in, const hit_record &record,
                         vec3_t &attenuation, ray &scattered) const = 0;

    [[nodiscard]] virtual color_t emit(float u, float v,
...
public:
    explicit lambertian(const color_t &albedo)
        : albedo_(std::make_shared<solid_color>(albedo)) {}

    explicit lambertian(std::shared_ptr<texture> tex)
        : albedo_(std::move(tex)) {}

    bool scatter(const ray &ray_in, const hit_record &record,
                 color_t &attenuation, ray &scattered) const override {
...
public:
    ...
    bool scatter(const ray &ray_in, const hit_record &record,
                 vec3_t &attenuation, ray &scattered) const override {
...
public:
    ...
    bool scatter(const ray &ray_in, const hit_record &record,
        ...
        const auto refraction =
            refract(ray_in.direction(), record.normal, eta_ratio);

        // 使用 Schlick's Approximation 计算反射率
        auto reflectance = [&]() {
            const float cos_theta =
                std::min(-ray_in.direction().dot(record.normal), 1.0f);
...
public:
    explicit light(const color_t &color)
        : albedo_(std::make_shared<solid_color>(color)) {}
    ...
    [[nodiscard]] color_t emit(float u, float v,
                               const point_t &point) const override {
